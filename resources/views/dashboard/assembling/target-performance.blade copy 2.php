                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    <div class="border border-green-200 bg-green-50 rounded-lg p-4">
                        <div class="flex justify-between items-start">
                            ...
                            <!-- <button class="toggle-switch" data-id="lifterA" data-status="on" onclick="toggleStatus('lifterA')">
                                <div class="flex items-center space-x-2">
                                    <svg class="w-4 h-4 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.12a1 1 0 00-.363 1.118l1.519 4.674c.3.921-.755 1.688-1.538 1.118l-3.976-2.12a1 1 0 00-1.175 0l-3.976 2.12c-.783.57-1.838-.197-1.538-1.118l1.519-4.674a1 1 0 00-.363-1.118l-3.976-2.12c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                                    </svg>
                        ...
                        <div class="flex justify-between items-start">
                            ...
                            >
                                <div class="flex items-center space-x-2">
                                    ...
                                    <span class="text-xs font-medium text-red-600">Off</span>
                                </div>
                            </button>
                        </div>
                        <div class="mt-3 space-y-1 text-sm">
...
<script>
    ...
    $(document).ready(function () {
        ...
        // Toggle Status Function (dibuat bisa diakses oleh button)
        window.toggleStatus = function (id) {
            ...
            if (currentStatus === 'on') {
                // Ubah ke OFF
                $statusSpan.text('Off');
                $statusSpan.css('color', '#dc2626');
                $svgIcon.css('fill', '#dc2626');
                $button.attr('data-status', 'off');

                // Update status visual lainnya
                $button.closest('.p-4').removeClass('bg-green-50 border-green-200').addClass('bg-red-50 border-red-200');
                $button.closest('.p-4').find('[class*="text-green-600"]').removeClass('text-green-600').addClass('text-red-600').text('Off');
            } else {
                // Ubah ke ON
                $statusSpan.text('On');
                $statusSpan.css('color', '#10b981');
                $svgIcon.css('fill', '#10b981');
                $button.attr('data-status', 'on');

                // Update status visual lainnya
                $button.closest('.p-4').removeClass('bg-red-50 border-red-200').addClass('bg-green-50 border-green-200');
                $button.closest('.p-4').find('[class*="text-red-600"]').removeClass('text-red-600').addClass('text-green-600').text('On');
            }

            console.log(`${id} toggled to ${$statusSpan.text()}`);
        };
        ...
        // Optional: Re-render chart saat tab diaktifkan
        $('.tab-button').on('click', function () {
            const tabId = $(this).data('tab');
