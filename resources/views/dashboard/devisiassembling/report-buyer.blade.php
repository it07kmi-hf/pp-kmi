...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('buyerSelector').addEventListener('change', function() {
    ...
    function updateData(period) {
        ...
        // Update ranking list
        const rankingList = document.getElementById('buyerRankingList');
        rankingList.innerHTML = '';

        sortedBuyers.forEach(([name, info], index) => {
            const rankingItem = document.createElement('div');
            rankingItem.className = 'flex items-center justify-between p-3 bg-gray-50 rounded-lg';
    ...
    function updateSelectedBuyer(buyerName) {
        ...
        // Update stats cards
        document.getElementById('selectedBuyerValue').textContent = `$${data.value.toLocaleString()}`;
        document.getElementById('selectedBuyerValueChange').textContent = `${data.change} vs bulan lalu`;
        document.getElementById('selectedBuyerQty').textContent = `${data.qty} Unit`;
        document.getElementById('selectedBuyerRank').textContent = `Ranking #${data.rank}`;
        
        const avgOrderValue = data.qty > 0 ? Math.round(data.value / data.qty) : 0;
        document.getElementById('selectedBuyerAvg').textContent = `$${avgOrderValue}`;

        // Update distribusi subtitle
        document.getElementById('distribusiSubtitle').textContent = `Output ${buyerName} per line assembling`;
    ...
    function switchDetailTab(tabName) {
        ...
        document.querySelectorAll('.detail-tab-button').forEach(btn => {
            btn.className = 'detail-tab-button px-6 py-3 font-medium text-gray-500 border-b-2 border-transparent hover:text-gray-700';
        });

        // Activate selected tab
        document.querySelector(`[data-tab="${tabName}"]`).className = 'detail-tab-button px-6 py-3 font-medium text-gray-700 border-b-2 border-green-500 bg-green-50';
